function [ O,P ] = GDUpdate_Multiplication_rank1(O,P,dpsi,Omax,cen,Ps,alpha,beta)
...
O(n1(1):n2(1),n1(2):n2(2)) = O(n1(1):n2(1),n1(2):n2(2))...
    + 1/max(max(abs(P)))*abs(P).*conj(P).*dpsi./(abs(P).^2+alpha);
P = P+1/Omax*(abs(O1).*conj(O1)).*dpsi./(abs(O1).^2+beta).*Ps;

% P = P+conj(O1).*dpsi/max(max(abs(O1).^2));%1/Omax*dP./(sumO+beta).*Ps(:,:,m);
% O(n1(1):n2(1),n1(2):n2(2)) = O(n1(1):n2(1),n1(2):n2(2))+conj(P).*dpsi/max(max(abs(P).^2));%
